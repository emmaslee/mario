...
Minim minim;
AudioPlayer coin, theme, gameover;
...
//terrain
PImage map1, mapyyy, superr, flag, mapy, bridge, spike, teleporter, ice, stone, treeTrunk, treeIntersect, treeMiddle, treeEndWest, treeEndEast, trampoline, hammer, thwompSleepy, space;
//lava animations
PImage[] lava;
...
void setup() {
  ...
  blueHeart = new Gif("frame_", "_delay-0.2s.gif", 2, 5, 0, 0, width, height);
  loadImages();
  loadWorld(map1);
  loadPlayer();
  mode = INTRO;
...
void makeButtons() {
  rectMode(CENTER);
  start = new Button("START", 130, 350, 150, 100, white, black);
  info = new Button("INFO", 130, 500, 150, 100, white, black);
  restart = new Button("HOME", 300, 450, 150, 100, red, blue);
  returnHome = new Button("HOME", 490, 100, 150, 100, white, red);
}
void drawWorld() {
  pushMatrix();
